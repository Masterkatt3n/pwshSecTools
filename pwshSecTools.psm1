...
# --- 4. Secure file overwrite + delete -----------------------------------
function Remove-SecureFile {
  ...
  if ($Path -match '^(C:\\$|C:\\Windows(\\|$)|C:\\Program Files(\\|$))') {
    Write-Error "Refusing to wipe critical system path: $Path"
    return
  }
  ...
  $tmp = Join-Path $file.DirectoryName ([guid]::NewGuid().ToString())
  Rename-Item -LiteralPath $file.FullName -NewName $tmp -Force
  [IO.File]::WriteAllBytes($tmp, [byte[]]@())
  Remove-Item -LiteralPath $tmp -Force
  Write-Host "File securely removed: $Path" -ForegroundColor DarkGreen
}

# --- 5. Clear Windows Recent + Jump Lists --------------------------------
function Clear-RecentItem {
  $RecentItemsPath            = Join-Path $env:APPDATA 'Microsoft\Windows\Recent'
  $AutomaticDestinationsPath  = Join-Path $RecentItemsPath 'AutomaticDestinations'
  $CustomDestinationsPath     = Join-Path $RecentItemsPath 'CustomDestinations'

  foreach ($p in @($RecentItemsPath, $AutomaticDestinationsPath, $CustomDestinationsPath)) {
    if (Test-Path $p) {
      Get-ChildItem -Path $p -File -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
    }
  }
  Write-Host "Recent items and Jump Lists cleared." -ForegroundColor DarkGreen
}
...
function Update-pwshSecToolsModule {
  ...
  foreach ($f in $files) {
    $src = if ($f -like '*.py') { Join-Path $srcPy $f } else { Join-Path $srcPS $f }
